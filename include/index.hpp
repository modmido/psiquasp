...
public:
    ...
    inline PetscInt	ContinueMLS();					//true so lone we reached the end of the first mls block
    ...
    PetscInt		NMls()			{ return NMLS; }			//!< Return the number of mls
};
...
{
    PetscInt	i,ret = 1 , upper = 1;
    
    PetscInt	newblockindices [firstmodedim];
    for(i=0; i < firstmodedim; i++)	newblockindices[i] = blockindices[i];
    
    if(down)		//start with downward pitch
    {
      PetscInt	i = down-1,j,pitch = 0;
      
      newblockindices[i]	= blockindices[i]-1;			//
      upper			= blocksizes[i][blockindices[i]-1];	//
    ...
    if(up)
    {
      ...
      upper2 = blocksizes[i][newblockindices[i]];
...
/**
 * @brief	Checks whether a while loop can be continued with the current Index. Says no at the end of the first mls block. Makes only sense in combination with InitializeGlobal().
 * 
 */
