...
PetscErrorCode TwoTLS::Setup(Vec* dm, Mat* AA)
{
    ...
    //parameters
    PetscInt		ntls0 = 2, dx0 = 2, ntls1 = 2, dx1 = 2, m0=4, dm0=4;
    ...
    PetscInt	qnumbers [8] = {0,0,0,1,0,0,0,0};                  //first tls type has a single excitation
...
PetscErrorCode MyOut::SetupMyOut(TwoTLS * system)
{
    ...
    ierr = n11file->SetupMLSDistFile(system,"n11.dat",&n11_0);CHKERRQ(ierr);			//distribution in the n11 number states P[n11,0,0]
...
PetscErrorCode CorrelationsFile::SetupMyGnFile(TwoTLS * sys, std::string name)
{
    ...
    ierr = n11secorder->SetupMLSGnfct(sys,&n01_0,2);CHKERRQ(ierr);				//computes g^(2) = <J_{10} J_{10} J_{01} J_{01}>/<J_{10} J_{01}>^2
    ierr = AddElem(n11secorder,"g(2)(n11)");CHKERRQ(ierr);
    
    ierr = n11thirdorder->SetupMLSGnfct(sys,&n01_0,3);CHKERRQ(ierr);			//computes g^(2) = <J_{10} J_{10} J_{10} J_{01} J_{01} J_{01}>/<J_{10} J_{01}>^3
