...
PetscErrorCode Laser::Setup(Vec* dm, Mat* AA)
{
    ...
    PetscReal		omega_mode = 2.0;
    PetscReal		omega_tls = 2.0;
    ...
    ierr = PetscOptionsGetReal(NULL,NULL,"-omega_tls",&omega_tls,NULL);CHKERRQ(ierr);		//and here we can reset the simulation parameters
    ierr = PetscOptionsGetReal(NULL,NULL,"-omega_mode",&omega_mode,NULL);CHKERRQ(ierr);
    ...
    ierr = AddParam("omega_mode",omega_mode); CHKERRQ(ierr);					//internal storage for parameters
    ierr = AddParam("omega_tls",omega_tls); CHKERRQ(ierr);
    ...
    ierr = TLSAdd(ntls,dx,dx,omega_tls*hbar);CHKERRQ(ierr);					//set the internal structure to ntls two-level systems, no offdiagonal trunctation so far
    ierr = ModeAdd(m0+1,dm0,omega_mode*hbar);CHKERRQ(ierr);					//add a single bosonic mode, the modes always have to be set after all mls degrees of freedom have been specified.
    ...
    ierr = AddTLSH0(*AA,NULL,NULL,1,omega_tls*PETSC_i);CHKERRQ(ierr);					//possible detuning between mode and TLS
    ierr = AddModeH0(*AA,d_nnz,o_nnz,0,0,omega_mode*PETSC_i);CHKERRQ(ierr);
...
PetscErrorCode ObservablesFile::SetupMyObsFile(Laser * system, std::string name)
{
    ...
    ierr = ppol10->SetupMlsPolarization(system,n10,0.0);CHKERRQ(ierr);	//computes the <J_{10}> expectation value, this is not hermitian, so PsiQuaSP prints real and imaginary part by default
    ierr = AddElem(ppol10,"Re<J_10>\t\tIm<J_10>");CHKERRQ(ierr);

    ierr = ppol01->SetupMlsPolarization(system,n01,0.0);CHKERRQ(ierr);	//computes the <J_{01}> expectation value,
    ...
    ierr = pmodepol->SetupModePolarization(system,0,0.0);CHKERRQ(ierr);	//computes <b>
