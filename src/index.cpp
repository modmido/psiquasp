...
Index::Index(PetscInt nlevels, PetscInt nummlsdims, PetscInt * mlspol, PetscInt N, PetscInt * dimlenghts, PetscInt modes, PetscInt *modedimlengths)
{
    ...
    for(i=0; i < firstmodedim; i++)	locdim_maxvalues[i] = NMLS;		//initialize it to NMLS, gets truncated later
  ...
  //-----------------------------------------------------------------
    ...
    {
      blocksizes_max[i]	= BinomDim(NMLS,firstmodedim-1-i);			//the number of blocks is given by the binomial coefficient, which is neat
   ...
   */
    ...
    blocksizes[firstmodedim-1][0] = NMLS + 1;					//for the last mls dimension there is only one block, given by the lowest order binomial coefficient (NMLS+1 over NMLS)
  ...
  //-----------------------------------------------------------------
    mls_dof		= BinomDim(NMLS,num_dims);
    
    total_dof		= mls_dof;				//the total number of dof is the mls_dof
  ...
  //-----------------------------------------------------------------
    ...
    PetscInt		check=0;
    mls_dof		= total_dof;				//the mls_dof is the total_dof ...
    for(i=0; i < modes; i++)
    {
      check += mls_dof % mode_dofs[i];				//little check if this acutally works
      mls_dof /= mode_dofs[i];					// ... divided by all the mode dofs
    }
    if(check)
    {
